...
module.exports = async function handler(req, res) {
  ...
  const roleDesc  = roleDescs[role]  || ''

  console.log('Sending invite to:', email, 'token:', token.slice(0, 8) + '...')
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      html: [
        ...
        '<strong style="color:#0f172a;">' + (invitedBy || 'A team member') + '</strong>',
        ' has invited you to join ',
        '<strong style="color:#0f172a;">' + orgName + '</strong>',
        ' on DNSMonitor as a <strong style="color:#1a6bff;">' + roleLabel + '</strong>.',
        '</p>',
        ...
        // CTA button — table-based so Outlook renders it correctly
        '<table cellpadding="0" cellspacing="0" border="0" style="margin-bottom:16px;">',
        '<tr><td style="background:#1a6bff;border-radius:8px;padding:0;">',
        '<a href="' + inviteUrl + '" target="_blank" style="display:inline-block;padding:13px 28px;font-size:14px;font-weight:700;color:#ffffff;text-decoration:none;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;">',
        ...
        '<a href="' + inviteUrl + '" style="color:#1a6bff;word-break:break-all;">' + inviteUrl + '</a>',
        '</p>',
        ...
        '<p style="font-size:11px;color:#cbd5e1;margin:4px 0 0;">If you weren\'t expecting this invitation, you can safely ignore this email.</p>',
        '</div>',
    ...
    console.log('Invite sent OK, id:', data?.id)
    return res.status(200).json({ success: true, id: data?.id })

  } catch (err) {
    console.error('Send invite exception:', err?.message)
